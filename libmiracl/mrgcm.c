...
// Tag should be 619cc5aefffe0bfa462af43c1699d050
	...
	hex2bytes(KT, K);
 	...
 	Debug_printf("Plaintext=\n");
	for (i=0;i<len;i++) Debug_printf("%02x",(unsigned char)M[i]);
	Debug_printf("\n");
	...
	Debug_printf("Ciphertext=\n");
	for (i=0;i<len;i++) Debug_printf("%02x",(unsigned char)C[i]);
	Debug_printf("\n");
        
	Debug_printf("Tag=\n");
	for (i=0;i<16;i++) Debug_printf("%02x",(unsigned char)T[i]);
	Debug_printf("\n");
	...
	gcm_finish(&g,T);
 	...
 	Debug_printf("Plaintext=\n");
	for (i=0;i<len;i++) Debug_printf("%02x",(unsigned char)P[i]);
	Debug_printf("\n");
